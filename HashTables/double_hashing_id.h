#ifndef DOUBLEHASHINGID_H
#define DOUBLEHASHINGID_H
...
class DoubleHashingID : public HashTable {
private:
    struct Slot {
    ...
    int hashFunction2(const std::string &key) const {
        int hashVal = 0;
        for (char c : key) {
            hashVal = (hashVal * 17 + c) % (capacity - 1);
        }
        return (hashVal % (capacity - 1)) + 1; // Debe ser mayor que 0
    }
...
public:
    DoubleHashingID(int capacity) : table(capacity), capacity(capacity), size(0) {}
