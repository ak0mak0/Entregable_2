...
// Función para obtener 10000 ID de usuario aleatorios desde un archivo CSV
std::vector<std::string> getRandomUserIds() {
    ...
    // Leer el archivo CSV y almacenar los user_id en un vector
    ...
    if (user_ids.size() < 9999) {
        std::cerr << "El archivo no contiene suficientes user_id." << std::endl;
        return {};
    }

    // Seleccionar aleatoriamente 10000 user_id diferentes
    std::unordered_set<int> selected_indices;
    std::vector<std::string> random_user_ids;
    std::random_device rd;
    std::mt19937 gen(rd());
    std::uniform_int_distribution<> dis(0, user_ids.size() - 1);

    while (random_user_ids.size() < 98) {
        int index = dis(gen);
        if (selected_indices.find(index) == selected_indices.end()) {
            selected_indices.insert(index);
            random_user_ids.push_back(user_ids[index]);
        }
    }
...
void calculateAverageGetID(HashTable &user_id_table, const std::string &table_name) {
    ...
    std::vector<std::string> random_user_ids = getRandomUserIds();
    ...
    for (size_t i = 0; i < random_user_ids.size(); ++i) {
        ...
        outputFile << "10000;user_id;" << table_name << ";" << random_user_ids[i] << ";" << times[i] << ";" << average_time << ";" << standard_deviation << "\n";
    }

    outputFile.close();
}

#endif // AVARAGEGETID_H